...
CONCEPTS_LIST = [
    ...
    "technique", "temps", "transindividuel", "utopie", "tristesse", "multitude", 
]
...
def verify_concepts():
    ...
    with open(os.path.join(NOTES_FOLDER, "verif_concepts.md"), "w", encoding="utf-8") as f:
        ...
        for file in note_samples:
            ...
            f.write(f"## {file}\n")
            if tags:
                f.write("**Concepts détectés :**\n")
                for tag in tags:
                    f.write(f"- {tag}\n")
            else:
                f.write("❌ Aucun concept détecté\n")
            f.write("\n" + "="*40 + "\n\n")
    ...
    for root, _, files in os.walk(NOTES_FOLDER):
        for file in files:
            if file.endswith(".md"):
                ...
                tags = detect_entities(extract_content(content))

                if tags:
                    with open(file_path, "a", encoding="utf-8") as f:
                        f.write(f"\n\n# Tags : {', '.join(tags)}")
                    print(f"✅ Tags ajoutés : {file}")

if __name__ == "__main__":
    print("🔍 Vérification de 30 notes aléatoires...")
    verify_concepts()

    confirmation = input("\nSouhaites-tu valider et ajouter les tags dans toutes les notes ? (oui/non) : ").strip().lower()
    if confirmation == "oui":
        print("\n✅ Ajout des tags dans toutes les notes en cours...")
        add_tags_to_notes()
        print("✅ Tags ajoutés avec succès !")
    else:
        print("❌ Opération annulée. Aucun tag n'a été ajouté.")
